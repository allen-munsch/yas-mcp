...
mod tests {
    ...
    fn make_endpoint_config() -> EndpointConfig {
        EndpointConfig {
            ...
            auth_type: crate::internal::config::AuthType::Bearer,
            headers: {
    ...
    fn test_new_creates_client() {
        let config = make_endpoint_config();
        let requester = HttpRequester::new(&config);
        assert!(requester.is_ok());
        let r = requester.unwrap();
        assert_eq!(r.service_cfg.base_url, "http://api.example.com");
    }

    #[test]
    fn test_new_with_empty_base_url() {
    ...
    fn test_new_with_headers() {
        ...
        assert!(requester.service_cfg.headers.contains_key("X-Custom"));
        assert_eq!(
            requester.service_cfg.headers.get("X-Custom").unwrap(),
            "value"
        );
    }

    #[test]
    fn test_build_route_executor_get() {
        let config = make_endpoint_config();
        let requester = HttpRequester::new(&config).unwrap();
        let route_config = crate::internal::requester::types::RouteConfig {
            path: "/users".into(),
            method: "GET".into(),
    ...
    #[test]
    fn test_build_route_executor_post() {
        let config = make_endpoint_config();
        let requester = HttpRequester::new(&config).unwrap();
        let route_config = crate::internal::requester::types::RouteConfig {
            path: "/projects".into(),
            method: "POST".into(),
            description: "Create project".into(),
            method_config: crate::internal::requester::types::MethodConfig {
                query_params: vec!["filter".into()],
                header_params: vec!["x-request-id".into()],
                ..Default::default()
            },
            ..Default::default()
        };
    ...
    #[test]
    fn test_build_route_executor_with_path_params() {
        let config = make_endpoint_config();
        let requester = HttpRequester::new(&config).unwrap();
        let route_config = crate::internal::requester::types::RouteConfig {
            path: "/users/{userId}/posts/{postId}".into(),
            method: "GET".into(),
            description: "Get user post".into(),
            ..Default::default()
        };
        let executor = requester.build_route_executor(&route_config);
        assert!(executor.is_ok());
    ...
    #[test]
    fn test_build_route_executor_unsupported_method() {
        let config = make_endpoint_config();
        let requester = HttpRequester::new(&config).unwrap();
        let route_config = crate::internal::requester::types::RouteConfig {
            path: "/data".into(),
    ...
    fn test_set_timeout() {
        let config = make_endpoint_config();
        let mut requester = HttpRequester::new(&config).unwrap();
        let result = requester.set_timeout(Duration::from_secs(60));
        assert!(result.is_ok());
    }
